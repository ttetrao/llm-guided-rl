...
- Batch: 1 stato per request, retry, ThreadPool; pacing 10s (Ollama cloud)
...
def call_with_retry(
    ...
    for attempt in range(retries):
        try:
            ...
            if content:
                print(f"  risposta grezza preview: {content}")
...
def run(
    ...
    if dry_run:
        print(
            ...
            f"model={model_id} host={ollama_host or os.environ.get('OLLAMA_HOST','https://ollama.com')}"
        )
    ...
    print(
        ...
        f"model {model_id}, lang {lang}, temp {temperature}"
    )

    if Client is None:
        print("ERRORE: pip install ollama mancante. Usa --dry-run per test senza API.")
        return rows
    client = get_ollama_client(host=ollama_host)
    if client is None:
        print("ERRORE: impossibile creare Ollama Client. Usa --dry-run")
        return rows
    if (ollama_host or os.environ.get("OLLAMA_HOST", "https://ollama.com")).startswith(
        "https://"
    ) and not os.environ.get("OLLAMA_API_KEY"):
        print("WARN: OLLAMA_API_KEY non impostata ma host è https://ollama.com.")
    ...
    def process_batch(batch: list[tuple[str, int]], batch_idx: int):
        ...
        resp = call_with_retry(
            client, full_prompt, model_id=model_id, temperature=temperature
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Query Ollama gpt-oss su FrozenLake slippery (Q* stocastica via V*)"
    )
    ...
    parser.add_argument("--ollama-host", type=str, default=None, dest="ollama_host")
