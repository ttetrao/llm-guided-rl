...
Formato JSON atteso (una riga per azione, 4 azioni per stato in mode q):
  id, seed, map_name, bucket, state, r, c, ch, action, v_true, v_llm, analysis
dove v_true = Q*(s,a) stocastico = Σ p·[r+γV*(s')] e v_llm = stima LLM.
...
def find_files(path):
    ...
    if p.is_file():
        return [str(p)]
    files = sorted(glob.glob(os.path.join(path, "**", "frozenlake_llm*.json"), recursive=True))
    files += sorted(glob.glob(os.path.join(path, "**", "frozenlake_llm*.csv"), recursive=True))
    if not files:
        files = sorted(glob.glob(os.path.join(path, "**", "*.json"), recursive=True))
        files += sorted(glob.glob(os.path.join(path, "**", "*.csv"), recursive=True))
        files = [f for f in files if "frozenlake" in os.path.basename(f).lower()]
    return files
...
def plot_mae_rmse_bucket(metrics, tag, gdir):
    ...
    fig.suptitle(f"Errore per bucket — {tag} (FrozenLake)", fontsize=13, fontweight="bold")
...
def main():
    ...
    files = [f for f in find_files(args.path) if "frozenlake" in os.path.basename(f).lower()
             or "llm" in os.path.basename(f).lower()]
    if not files and os.path.isfile(args.path):
        files = [args.path]  # fallback: accetta comunque un file json diretto
    if not files:
    ...
    log_lines = []
    log_lines.append("=" * 70)
    log_lines.append("VALUTAZIONE LLM FrozenLake slippery (Q* stocastico) — v3")
    log_lines.append("corrispettivo di evaluate_llm.py per FrozenLake | 4 azioni | filtro per stato | argmax tie-aware | regret | Δk")
    ...
    for f in files:
        ...
        # ---- plot ----
        plot_scatter(df_val, metrics, f"v_true vs v_llm — {tag} (FrozenLake slippery)", str(gdir / f"{tag}_scatter.png"))
        plot_residuals(df_val, metrics, f"Residui v_llm − v_true — {tag} (FrozenLake)", str(gdir / f"{tag}_residuals.png"))
        ok_k = plot_k_error(metrics, gamma, f"Errore sull'esponente k — {tag} (FrozenLake)", str(gdir / f"{tag}_k_error.png"))
        plot_action_acc(act, f"Accuratezza azione — {tag} (FrozenLake, 4 azioni)", str(gdir / f"{tag}_accuracy.png"))
        plot_mae_rmse_bucket(metrics, tag, gdir)
        plot_confusion(act, f"Confusion matrix — {tag} (FrozenLake)", str(gdir / f"{tag}_confusion.png"))
        ...
        if metrics.get("k_n", 0) > 0:
            ...
            log_lines.append("Δk: non calcolabile (valori non on-grid con questo gamma — normale per FrozenLake slippery)")
